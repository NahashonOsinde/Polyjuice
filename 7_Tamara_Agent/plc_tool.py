...
class PLCInterface:
    ...
    def write_payload_to_plc(self, payload: InputPayload) -> None:
        ...
        chip_val = 1 if payload.chip_id == ChipID.HERRINGBONE else 0
        data = bytearray(2)
        if snap7:
        ...
        mani_val = 0 if payload.manifold == Manifold.SMALL else 1
        data = bytearray(2)
        if snap7:
        ...
        # STATUS (INT) 2=RUN,3=CLEAN,4=PRESSURE_TEST
        mode_map = {Mode.RUN: 2, Mode.CLEAN: 3, Mode.PRESSURE_TEST: 4}
