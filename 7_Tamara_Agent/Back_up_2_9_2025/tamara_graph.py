...
Process references from your PLC FBs (for context-only docs in prompts/UX):
...
Run:
  $ python tamara_graph.py            # REPL
  $ python tamara_graph.py --draw     # also saves ./tamara_graph.png (and previews in notebooks)
...
# log = logging.getLogger("tamara.graph")
...
def _ingest_if_needed(persist_directory: str, txt_dir: str) -> Chroma:
    ...
    vs = Chroma.from_documents(splits, embedding=embeddings, persist_directory=persist_directory)
    vs.persist()
    return vs
    ...
    if "pause" in user_text:
        ...
        try:
            ...
            if status_code == 2:  # Running
                ...
                log.info(f"Cannot pause - status code {status_code} is not Running(2)")
        ...
        try:
            ...
            if status_code == 2:  # Running but paused
                ...
                log.info(f"Cannot resume - status code {status_code} is not Running(2)")
        ...
        try:
            ...
            if status_code in [2, 3, 4]:  # Running, Cleaning, or Pressure Test
                ...
                log.info(f"Cannot stop - status code {status_code} not in [2,3,4] (Running/Cleaning/PTest)")
        ...
        try:
            ...
            status_messages = {
                0: "Initializing - System startup in progress",
                1: "Ready - System is ready for operation",
                2: "Running - Normal operation in progress",
                3: "Cleaning - Clean cycle in progress",
                4: "Pressure Test - Testing system pressure",
                5: "Safe Purge - System purging in progress",
                6: "Faulted - System error detected",
                7: "Reset - System reset in progress",
                8: "E-Stop - Emergency stop activated"
            }
