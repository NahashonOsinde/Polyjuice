...
persistent_directory = os.path.join(db_dir, "chroma_db_InsideTxWeb_firecrawl")
...
def create_vector_store():
    ...
    loader = FireCrawlLoader(
        api_key=api_key, url="https://www.insidetx.com", mode="scrape")
    docs = loader.load()
    ...
    rec_split_docs = rec_char_splitter.split_documents(docs)

    # Display information about the split documents
    print("\n--- Document Chunks Information ---")
    print(f"Number of document chunks: {len(rec_split_docs)}")
    print(f"Sample chunk:\n{rec_split_docs[0].page_content}\n")
    ...
    db = Chroma.from_documents(
        rec_split_docs, embeddings, persist_directory=persistent_directory
    )
...
query = "What is Tamara?" #What new products are announced on insidetx.com
